...
export default {
  ...
  methods: {
    ...
    init(value){
      teamApi.awardList({ size:9999, businessType: value },{ token:this.token, showLoading:!this.orderId }, ).then(res=>{
        if(res.code==0){

          if(this.orderId){
              if(res.data.records.find(item=>item.businessId==this.orderId)){
      ...
      teamApi.awardCounts({ businessType: value },{ token:this.token, showLoading:!this.orderId }).then(res=>{
        if(res.code==0){
